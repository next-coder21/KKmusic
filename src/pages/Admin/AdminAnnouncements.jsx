...
const TC = {
  ...
  new_release: { Icon: FiMusic,       label: 'Release',     color: '#2dd4bf' },
...
const ST = {
  ...
  draft:     { dot: '#374151', label: 'Draft',        color: '#4b5563' },
};

const getStatus = (a) => a.sent_at ? 'sent' : a.scheduled_at ? 'scheduled' : 'draft';

/* ─── Section label ─────────────────────────────────────── */
const SL = ({ children, color = '#ec4899' }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
    <span style={{ width: 3, height: 14, borderRadius: 2, background: color, flexShrink: 0 }} />
    <span style={{ fontSize: 10, color: '#4b5563', letterSpacing: '0.14em', textTransform: 'uppercase', fontWeight: 600 }}>{children}</span>
...
const iSt = {
  ...
  padding: '9px 12px', color: '#f9fafb', fontSize: 13,
  outline: 'none', fontFamily: 'inherit', transition: 'border-color 0.12s',
};
const onFo = e => e.target.style.borderColor = '#ec4899';
const onBl = e => e.target.style.borderColor = '#1f1f1f';
        ...
        <div className="admin-split-left" style={card}>
            ...
            <div style={{ display: 'flex', gap: 5 }}>
              {['all', 'draft', 'scheduled', 'sent'].map(f => (
                <button key={f} onClick={() => setFilter(f)} style={{
                  ...
                  border: `1px solid ${filter === f ? '#ec4899' : '#1a1a1a'}`,
                  background: filter === f ? 'rgba(236,72,153,0.1)' : 'transparent',
                  color: filter === f ? '#ec4899' : '#4b5563',
                  fontSize: 10, fontWeight: 600, cursor: 'pointer', textTransform: 'uppercase',
                  letterSpacing: '0.06em', transition: 'all 0.12s', fontFamily: 'inherit',
                }}>{f}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '8px' }}>
            {loading ? [...Array(5)].map((_, i) => (
              <div key={i} style={{ height: 68, borderRadius: 8, background: '#141414', marginBottom: 5, animation: 'shimmer 1.4s ease-in-out infinite' }} />
            )) : filtered.length === 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', opacity: 0.3 }}>
                <FiLayers size={28} style={{ color: '#4b5563', marginBottom: 10 }} />
                <p style={{ fontSize: 12, color: '#4b5563', margin: 0 }}>No broadcasts found</p>
              </div>
            ) : filtered.map((ann, i) => {
              ...
              return (
                <motion.div key={ann.id}
                  ...
                  style={{
                    ...
                    border: `1px solid ${active ? 'rgba(236,72,153,0.25)' : '#1a1a1a'}`,
                    borderLeft: `3px solid ${active ? '#ec4899' : sc.dot}`,
                    background: active ? 'rgba(236,72,153,0.05)' : 'transparent',
                  }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 2 }}>
                      ...
                      <span style={{ fontSize: 9, color: '#1f2937' }}>·</span>
                      <span style={{ fontSize: 9, color: '#374151', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                        {ann.target === 'all' ? 'All' : ann.target === 'verified' ? 'Verified' : `${ann.target_emails?.length || 0} users`}
                      </span>
                    </div>
                    <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: '#e5e7eb', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ann.title}</p>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flexShrink: 0 }}>
                    <button onClick={e => { e.stopPropagation(); handleDuplicate(ann); }}
                      style={{ padding: 4, background: 'none', border: 'none', cursor: 'pointer', color: '#374151', borderRadius: 4, display: 'flex' }}
          ...
          <AnimatePresence mode="wait">
            ...
            {viewMode === 'none' && (
              <motion.div key="none" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', opacity: 0.25 }}>
                <div style={{ width: 52, height: 52, borderRadius: '50%', border: '2px dashed #374151', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14 }}>
                  <FiRadio size={22} style={{ color: '#374151' }} />
                </div>
                <p style={{ fontSize: 12, color: '#374151', margin: 0 }}>Select or create a broadcast</p>
              </motion.div>
            )}
            ...
            {viewMode === 'preview' && (
              <motion.div key="preview" initial={{ x: 24, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -16, opacity: 0 }}
                ...
                <div style={{ padding: '12px 18px', borderBottom: '1px solid #1a1a1a', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                  <button onClick={() => setViewMode('none')} style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'none', border: 'none', cursor: 'pointer', color: '#ec4899', fontSize: 12, fontFamily: 'inherit', fontWeight: 600 }}>
                    <FiChevronRight size={12} style={{ transform: 'rotate(180deg)' }} /> Back
                  </button>
                  <span style={{ fontSize: 10, color: '#374151', letterSpacing: '0.14em', textTransform: 'uppercase' }}>Preview</span>
                ...
                <div style={{ flex: 1, overflowY: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 32, background: '#0a0a0a' }}>
                  <div style={{ width: '100%', maxWidth: 360, background: '#0f0f0f', border: '1px solid #1a1a1a', borderRadius: 14, overflow: 'hidden', boxShadow: '0 24px 48px rgba(0,0,0,0.5)' }}>
                      ...
                      {(() => {
                        ...
                        return (
                          <div style={{ display: 'flex', gap: 10, marginBottom: 12, alignItems: 'flex-start' }}>
                            ...
                            <div>
                              <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: '#f9fafb' }}>{formData.title || 'Broadcast Title'}</p>
                              <p style={{ margin: '2px 0 0', fontSize: 10, color: '#374151', letterSpacing: '0.1em', textTransform: 'uppercase' }}>KK Music Team</p>
                            </div>
                          </div>
                        );
                      })()}
                      <p style={{ margin: '0 0 14px', fontSize: 13, color: '#6b7280', lineHeight: 1.65, whiteSpace: 'pre-line' }}>{formData.body || 'Message content...'}</p>
                      {formData.action_url && (
                        <button style={{ width: '100%', padding: 10, borderRadius: 8, background: 'linear-gradient(135deg,#ec4899,#6366f1)', color: '#fff', border: 'none', fontSize: 12, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer' }}>
                          {formData.action_label || 'Action'}
                        </button>
                      )}
                    </div>
                    <div style={{ padding: '8px 18px', borderTop: '1px solid #1a1a1a', background: '#0c0c0c' }}>
                      <p style={{ margin: 0, fontSize: 10, color: '#374151', textAlign: 'center', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                        Target: <span style={{ color: '#ec4899' }}>{formData.target === 'all' ? 'All users' : formData.target === 'verified' ? 'Verified' : `${formData.target_emails.length} specific`}</span>
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <motion.div key="compose" initial={{ x: 24, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -16, opacity: 0 }}
                ...
                <div style={{ padding: '12px 18px', borderBottom: '1px solid #1a1a1a', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <FiZap size={13} style={{ color: '#ec4899' }} />
                    <span style={{ fontSize: 14, fontWeight: 700, color: '#f9fafb' }}>{viewMode === 'edit' ? 'Edit Broadcast' : 'New Broadcast'}</span>
                  </div>
                  <button onClick={() => setViewMode('none')} style={{ padding: 5, background: 'transparent', border: 'none', cursor: 'pointer', color: '#374151', display: 'flex' }}>
                    <FiX size={15} />
                  </button>
                </div>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '20px 20px 100px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                      <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                          <label style={{ fontSize: 10, color: '#4b5563', letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 600 }}>Title *</label>
                          <span style={{ fontSize: 10, color: formData.title.length > 240 ? '#f87171' : '#374151' }}>{formData.title.length}/255</span>
                        </div>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: 10, color: '#4b5563', letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 8 }}>Category</label>
                          ...
                          {Object.entries(TC).map(([key, cfg]) => {
                            ...
                            return (
                              <button key={key} onClick={() => setFormData(p => ({ ...p, type: key }))}
                                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 11px', borderRadius: 7, border: `1px solid ${active ? cfg.color + '40' : '#1f1f1f'}`, background: active ? cfg.color + '12' : 'transparent', color: active ? cfg.color : '#4b5563', cursor: 'pointer', transition: 'all 0.12s', fontSize: 11, fontWeight: 600, fontFamily: 'inherit', letterSpacing: '0.04em' }}>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: 10, color: '#4b5563', letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 5 }}>Body</label>
                  ...
                  <div style={{ marginBottom: 26 }}>
                    <SL color="#60a5fa">Banner Image</SL>
                    <div onClick={() => fileRef.current.click()}
                      style={{ border: `2px dashed ${imagePreview ? '#ec4899' : '#1f1f1f'}`, borderRadius: 10, padding: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', cursor: 'pointer', background: imagePreview ? 'rgba(236,72,153,0.04)' : 'transparent', transition: 'all 0.12s' }}>
                      ...
                      {imagePreview ? (
                        ...
                        <div style={{ textAlign: 'center' }}>
                          <FiImage size={24} style={{ color: '#374151', marginBottom: 8 }} />
                          <p style={{ margin: 0, fontSize: 11, color: '#374151', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Click to upload · PNG / JPG · Max 2MB</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom: 26 }}>
                      ...
                      {[{ id: 'all', label: 'All Users', Ic: FiUsers }, { id: 'verified', label: 'Verified', Ic: FiShield }, { id: 'specific', label: 'Specific', Ic: FiUser }].map(t => {
                        ...
                        return (
                          <button key={t.id} onClick={() => setFormData(p => ({ ...p, target: t.id }))}
                            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, padding: '12px 8px', borderRadius: 9, border: `1px solid ${active ? 'rgba(236,72,153,0.3)' : '#1a1a1a'}`, background: active ? 'rgba(236,72,153,0.07)' : 'transparent', cursor: 'pointer', transition: 'all 0.12s', fontFamily: 'inherit' }}>
                            <t.Ic size={16} style={{ color: active ? '#ec4899' : '#374151' }} />
                            <span style={{ fontSize: 10, color: active ? '#ec4899' : '#4b5563', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase' }}>{t.label}</span>
                          </button>
                        );
                      })}
                    </div>
                    {formData.target === 'specific' && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}>
                        <div style={{ background: '#0a0a0a', border: '1px solid #1f1f1f', borderRadius: 8, padding: 8, minHeight: 72, display: 'flex', flexWrap: 'wrap', gap: 5, alignContent: 'flex-start' }}>
                          {formData.target_emails.map(em => (
                            <span key={em} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'rgba(236,72,153,0.1)', border: '1px solid rgba(236,72,153,0.2)', borderRadius: 5, padding: '3px 7px', fontSize: 11, color: '#ec4899' }}>
                              {em} <FiX size={10} style={{ cursor: 'pointer' }} onClick={() => removeEmail(em)} />
                            </span>
                          ))}
                          <input style={{ background: 'transparent', border: 'none', outline: 'none', fontSize: 12, color: '#f9fafb', flex: 1, minWidth: 180, fontFamily: 'inherit' }}
                      ...
                      {[{ val: true, label: 'Send immediately' }, { val: false, label: 'Schedule later' }].map(opt => {
                        ...
                        return (
                          <button key={String(opt.val)} onClick={() => setFormData(p => ({ ...p, send_immediately: opt.val }))}
                            style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 12px', borderRadius: 8, border: `1px solid ${active ? 'rgba(245,158,11,0.3)' : '#1a1a1a'}`, background: active ? 'rgba(245,158,11,0.07)' : 'transparent', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.12s' }}>
                            <span style={{ fontSize: 12, color: active ? '#f9fafb' : '#4b5563', fontWeight: active ? 600 : 400 }}>{opt.label}</span>
                ...
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '12px 18px', background: '#0f0f0f', borderTop: '1px solid #1a1a1a', display: 'flex', justifyContent: 'flex-end', gap: 8, zIndex: 10 }}>
        ...
        {showConfirm && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowConfirm(false)}
              style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.88)', backdropFilter: 'blur(5px)' }} />
            <motion.div initial={{ scale: 0.93, opacity: 0, y: 12 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.93, opacity: 0, y: 12 }}
              transition={{ type: 'spring', stiffness: 300, damping: 24 }}
              style={{ position: 'relative', zIndex: 10, ...card, width: '100%', maxWidth: 400, padding: 28, boxShadow: '0 40px 80px rgba(0,0,0,0.7)' }}>
              <div style={{ position: 'absolute', top: 0, left: 32, right: 32, height: 1, background: 'linear-gradient(90deg,transparent,rgba(236,72,153,0.4),transparent)' }} />
              <div style={{ width: 44, height: 44, borderRadius: 11, background: 'rgba(236,72,153,0.1)', border: '1px solid rgba(236,72,153,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px', color: '#ec4899' }}>
                <FiSend size={18} />
              </div>
              <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: 18, fontWeight: 800, textAlign: 'center', margin: '0 0 8px', color: '#f9fafb', letterSpacing: '-0.02em' }}>Confirm Transmission</h2>
              <p style={{ textAlign: 'center', fontSize: 13, color: '#6b7280', margin: '0 0 20px', lineHeight: 1.6 }}>
                Sending <span style={{ color: '#e5e7eb', fontWeight: 600 }}>"{formData.title}"</span> to{' '}
                <span style={{ color: '#ec4899', fontWeight: 700, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 8, marginBottom: 20 }}>
                {[['Type', TC[formData.type]?.label || formData.type], ['Target', formData.target], ['Image', imagePreview ? 'Yes' : 'No'], ['Send', formData.send_immediately ? 'Now' : 'Scheduled']].map(([k, v]) => (
                  <div key={k} style={{ padding: '7px 10px', borderRadius: 7, background: '#141414', border: '1px solid #1a1a1a', textAlign: 'center' }}>
                    <p style={{ margin: '0 0 2px', fontSize: 9, color: '#374151', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{k}</p>
                    <p style={{ margin: 0, fontSize: 11, color: '#e5e7eb', fontWeight: 600, textTransform: 'capitalize' }}>{v}</p>
                  </div>
                ))}
              </div>
