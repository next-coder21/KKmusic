...
const AdminArtists = ({ api }) => {
  ...
  return (
    <div>
      <PageHeader title="Artists" subtitle={`${artists.length} artists`} action={<Btn icon={FiPlus} onClick={() => { setEditingArtist(null); setForm({name:'',bio:'',image_url:''}); setShowModal(true); }}>Add Artist</Btn>} />
      <div style={{ ...card, overflow: 'hidden' }}>
        <TableWrap>
          ...
          {loading ? <SkeletonRows cols={4} rows={itemsPerPage} /> : (
            <tbody>
              {paginated.length === 0 ? <tr><td colSpan={4}><Empty message="No artists found" /></td></tr> :
                paginated.map(a => (
                <Tr key={a.id}>
                  <Td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <Cover src={a.image_url} size={36} radius={18} fallback={<FiUser size={14} style={{ color: '#374151' }} />} />
                      <div>
                        <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: '#e5e7eb' }}>{a.name}</p>
                        <p style={{ margin: '2px 0 0', fontSize: 11, color: '#374151', maxWidth: 240, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.bio || 'No bio'}</p>
                      </div>
                    </div>
                  </Td>
                  ...
                  <Td right>
                    <div style={{ display:'flex',gap:4,justifyContent:'flex-end' }}>
                      <button onClick={()=>openEdit(a)}
                        style={{ padding: '5px 8px', borderRadius: 6, background: 'transparent', border: '1px solid transparent', cursor: 'pointer', color: '#374151', transition: 'all 0.12s', display: 'inline-flex' }}
                        onMouseEnter={e => { e.currentTarget.style.color = '#3b82f6'; e.currentTarget.style.borderColor = 'rgba(59,130,246,0.2)'; e.currentTarget.style.background = 'rgba(59,130,246,0.08)'; }}
                      ...
                      <button onClick={() => del(a.id)} style={{ padding: '5px 8px', borderRadius: 6, background: 'transparent', border: '1px solid transparent', cursor: 'pointer', color: '#374151', transition: 'all 0.12s', display: 'inline-flex' }}
                        onMouseEnter={e => { e.currentTarget.style.color = '#ef4444'; e.currentTarget.style.borderColor = 'rgba(239,68,68,0.2)'; e.currentTarget.style.background = 'rgba(239,68,68,0.08)'; }}
                        onMouseLeave={e => { e.currentTarget.style.color = '#374151'; e.currentTarget.style.borderColor = 'transparent'; e.currentTarget.style.background = 'transparent'; }}>
                        <FiTrash2 size={13} />
                      </button>
                    </div>
      ...
      <Modal open={showModal} onClose={() => { setShowModal(false); setEditingArtist(null); }} title={editingArtist?`Edit Artist — "${editingArtist.name}"`:"Add Artist"}>
        ...
        <ModalFooter onCancel={() => { setShowModal(false); setEditingArtist(null); }} onSubmit={saveArtist} loading={saving} submitLabel={editingArtist?"Update Artist":"Add Artist"} />
      </Modal>
    </div>
  );
};
